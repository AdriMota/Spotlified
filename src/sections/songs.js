import { getSongsForArtist, searchSongs } from '../api.js'
import { setSongList, playSong } from './player.js'
import { getFavorites, toggleFavorite, isInFavorite } from './favorites.js'

// Les tags dont nous avons besoin pour afficher les chansons
const songsSection = document.querySelector('#songs')
const songsSectionTitle = songsSection.querySelector('h2')
...
function renderSong(song, songs) {
  ...
  newSong.querySelector('.play-button').addEventListener('click', (event) => {
  ...
  newSong.querySelector('.favorite-button').addEventListener('click', (event) => {
    event.preventDefault()

    toggleFavorite(song)
    toggleFavoriteIcon(event.target, song) // on passe le target du click, à savoir l'icône
  })
